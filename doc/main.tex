\newcommand{\KEYWORDS}{Evolution\"are Algorithmen, eindimensionalen Behälter-
...
\begin{lstlisting}[morekeywords={while,do,return,for}, escapechar=!, caption=Evolutionärer Bin-Packing-Algorithmus, label=lis:cycle]
...
Im Verlauf der Untersuchungen von E. Falkenauer mit dem HGGA konnte diese theoretisch optimale Lösung für alle Instanzen außer \texttt{u120\_08}, \texttt{u120\_19}, \texttt{u250\_07}, \texttt{u250\_12}, \texttt{u250\_13}, \texttt{t60\_07} und \texttt{t60\_18} auch praktisch ermittelt werden. Für die eben genannten Instanzen wurde dagegen eine Lösung gefunden, die jeweils einen Behälter mehr benötigt als die theoretische minimale Lösung. \parencite[19-22]{Falkenauer:1996} 
    ...
    Das primäre Ziel bei der Untersuchung des Algorithmus war die Ermittlung einer sinnvollen Konfiguration zur effektiven Bearbeitung der in \autoref{sec:instances} beschriebenen Probleminstanzen. Zu diesem Zweck wurde die in \autoref{lis:config} abgebildete Standardkonfiguration festgelegt. Sämtliche in den Abschnitten \ref{subsec:test-decfunc} bis \ref{subsec:test-qualfunc} beschriebenen Experimente nutzen diese Konfiguration und variieren ausschließlich die Einstellung des jeweils untersuchten Parameters. 
    \newpage
    \begin{lstlisting}[morekeywords={}, escapechar=!, caption=Standardkonfiguration für die Evaluierungsexperimente, label=lis:config]
 !$\textbf{Populationsgröße}$!      !$\leftarrow$!  !$\textit{500}$!
...
\end{lstlisting}
    ...
    Beim Experiment zur Untersuchung der Rekombinationsverfahren wurde analog zur Untersuchung der Mutationsoperatoren vorgegangen (vgl. \autoref{subsec:mutation-comparison}). Als Eingabe-Datensatz diente die Probleminstanz \texttt{u120\_15}. Das Ergebnis des Vergleichs ist in \autoref{plt:recombinations} dargestellt. In der Abbildung wird deutlich, dass die Ermittlung einer minimalen Lösung unter Verwendung der Ordnungsrekombination wesentlich länger dauerte als mit der Abbildungsrekombination und nur in 73\% der Testläufe überhaupt erfolgreich war.
